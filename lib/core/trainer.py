...
def do_train(cfg, model, data_loader, loss_factory, optimizer, epoch,
    ...
    #这里会触发之前实例化的dataset的一个函数__getitem__()CocoKeypoint的__getitem__()会返回4个返回值。img, target_list, mask_list, joints_list就是对应着trainer.py中的(images, heatmaps, masks, joints)
    for i, (images, heatmaps, masks, joints) in enumerate(data_loader):
        ...
        #最终会计算损失，loss_factory实际上就是之前提到的MultiLossFactory函数。
        # loss = loss_factory(outputs, heatmaps, masks)在执行MultiLossFactory的forward函数。
        ...
        # compute gradient and do update step,更新参数
        optimizer.zero_grad()
        if fp16:
...
def _get_loss_info(loss_meters, loss_name):
    ...
    for i, meter in enumerate(loss_meters):
        ...
        # msg += 'Stage{i}-{name}: {meter.3e} ({meter.avg:.3e})\t'.format(
        #     i=i, name=loss_name, meter=meter
        # )
