...
def get_file_name_by_id(image_id):
    """
    根据 image_id 获取对应图片的文件名，不包括路径和后缀.
    :param image_id: 图片 ID
    :return: 图片名称（不包括 .jpg 后缀）
    """
    annotations_path = "F:\coco\\annotations\\person_keypoints_val2017.json"
    ...
    # print(file_name)
    if file_name:
            return file_name.split("\\")[-1].replace(".png", "")
    return None
...
def main():
    ...
    for i, (images, annos) in enumerate(data_loader):
        ...
        # 不再限制每多少步保存一次
        image_name=get_file_name_by_id(i)
        newoutpurdir=os.path.join(final_output_dir,'paper_result_1111')
        prefix = os.path.join(newoutpurdir, f'result_{image_name}')
