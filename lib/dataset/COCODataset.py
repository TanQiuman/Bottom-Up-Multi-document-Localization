...
from pycocotools.coco import COCO
from .cocodocumenteval import COCODocumenteval
...
class CocoDataset(Dataset):
    ...
    def evaluate(self, cfg, preds, scores, output_dir,
        ...
        for idx, _kpts in enumerate(preds):
            ...
            image_id = self.coco.loadImgs(img_id)[0]['id']  # 从 COCO 注释文件中获取 image_id
            # 从 file_name 中提取出 'filename'（去掉 '.jpg' 部分）
            for idx_kpt, kpt in enumerate(_kpts):
                ...
                # )
                # 提取文件名中的数字部分
                file_id = int(file_name.split('\\')[-1].split('.')[0])  # 从路径中提取文件名并去掉扩展名
                kpts[file_id].append(
                    {
                        'keypoints': kpt[:, 0:3],
                        'score': scores[idx][idx_kpt],
    ...
    def _do_python_keypoint_eval(self, res_file, res_folder):
        ...
        coco_eval.params.kpt_oks_sigmas=np.array([0.1,0.1,0.1,0.1],dtype=np.float32)
